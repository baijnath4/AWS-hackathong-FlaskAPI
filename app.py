...
def save_report_pdf():
    try:
        ...
        status = save_text_to_pdf(text, filename)
...
def filter_crisis():
    try:
        ...
        # Call the actual filtering logic from your module
        records = filter_by_date(date)

        return jsonify({
            "status": "ok",
            "received_date": date,
            "records": records  # Assuming this is list[dict] or a serializable structure
        })
